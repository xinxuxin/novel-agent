...
默认入口收敛为 `章节成文`。顶栏只保留当前写作主入口、`高级功能` 下拉和 `设置`，旧的整理素材、章节细纲、故事圣经、成本、评测和导入导出仍保留在高级入口中。

章节成文页只暴露核心动作：

- 导入或粘贴活动设定文件。
- 导入或粘贴本章细纲。
- 点击 `确认当前细纲`，把当前章节计划标记为 accepted。
- 点击 `生成本章正文`，运行固定单章写作链。
- 在人工确认区 `保存为版本`，再单独 `设为正式正文`。

默认页不再展示多草稿对比或多模型检查。生成候选稿、审稿报告和终检报告都保持非正式状态，直到用户明确保存或设为正式正文。
